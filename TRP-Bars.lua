...
function main()
	...
	if IP:find(TRINITYGTA_IP["RPG"]) or IP:find(TRINITYGTA_IP["RP1"]) or IP:find(TRINITYGTA_IP["RP2"]) then
		...
		--
		TRINITYGTA = true
		sampAddChatMessage(TAG .. "{FFFFFF}Ñêðèïò çàïóùåí. Àâòîð: {FFCC00}eweest{FFFFFF}. Âåðñèÿ: {FFCC00}" .. thisScript().version .. "{FFFFFF}. Ïîìîùü {FFCC00}" .. CMD_HELP, 0xFFCC00)
	else
		sampAddChatMessage(TAG .. "{FFFFFF}Ñêðèïò " .. thisScript().name .. " (v" .. thisScript().version .. ") íå çàïóùåí, òàê êàê âû èãðûåòå íå íà Trinity GTA.", 0xFFCC00)
		TRINITYGTA = false 
	end
	----
	...
	sampRegisterChatCommand("bars", function(cmd) -- HELP
		if (cmd == "position" or cmd == "pos") then -- POSITION
			sampAddChatMessage(TAG .. "{FFFFFF}Äëÿ ñîõðàíåíèÿ ïîçèöèè, íàæìèòå {FFCC00}ëåâóþ êíîïêó ìûøè {FFFFFF}èëè{FFCC00} ENTER.", 0xFFCC00)
			setPosition = true
		-- SETTINGS CMD
		elseif (cmd == "number" or cmd == "num") then -- NUMBERS
			if (DB["TYPE"] == 1) then
				...
				if DB["NUMBER"] then
					...
					sampAddChatMessage(TAG .."{FFFFFF}Ïîêàçàòåëü ïîòðåáíîñòåé ïåðñîíàæà â öèôðàõ {32FF32}âêëþ÷åí.", 0xFFCC00)
					DB["NUMBER"] = true
				else
					addOneOffSound(0.0, 0.0, 0.0, SOUNDS["CANCEL"])	
					sampAddChatMessage(TAG .."{FFFFFF}Ïîêàçàòåëü ïîòðåáíîñòåé ïåðñîíàæà â öèôðàõ {FF3232}îòêëþ÷åí.", 0xFFCC00)
					DB["NUMBER"] = false
				end
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ïîêàçàòåëü ïîòðåáíîñòåé ïåðñîíàæà â öèôðàõ äîñòóïåí òîëüêî â ñòèëå {FFCC00}#1 'Ñòàìèíà'{FFFFFF}.", 0xFFCC00)
			end
			saveDB()
		elseif (cmd == "view" or cmd == "vw") then -- VIEW MODE
			...
			if DB["VIEW"] then
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
				sampAddChatMessage(TAG .."{FFFFFF}Ïîòðåáíîñòè ïåðñîíàæà {32FF32}âêëþ÷åíû.", 0xFFCC00)
				DB["VIEW"] = true
			else
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["CANCEL"])	
				sampAddChatMessage(TAG .."{FFFFFF}Ïîòðåáíîñòè ïåðñîíàæà {FF3232}îòêëþ÷åíû.", 0xFFCC00)
				DB["VIEW"] = false
			end
			saveDB()
		elseif (cmd == "rotation" or cmd == "rt") then -- VIEW MODE
			if DB["ROT"] ~= "HORIZONTAL" and (cmd == "rot" or cmd == "rt") then
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
				sampAddChatMessage(TAG .."{FFFFFF}Âû àêòèâèðîâàëè {FFCC00}Ãîðèçîíòàëüíûé{FFFFFF} ðåæèì.", 0xFFCC00)
				DB["ROT"] = "HORIZONTAL"
			else
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["CANCEL"])	
				sampAddChatMessage(TAG .."{FFFFFF}Âû àêòèâèðîâàëè {FFCC00}Âåðòèêàëüíûé{FFFFFF} ðåæèì.", 0xFFCC00)
				DB["ROT"] = "VERTICAL"
			end
			saveDB()
		-- BARS STYLE
		elseif (cmd == "type 1") then -- TYPE 1
			if (DB["TYPE"] ~= 1) then
				DB["TYPE"] = 1
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà èçìåíåí. Âàðèàíò: {FFCC00} #1 'Ñòàìèíà'.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà {FFCC00}#1 'Ñòàìèíà'{FFFFFF} óæå èñïîëüçóåòñÿ!", 0xFFCC00)
		...
		elseif (cmd == "type 2") then -- TYPE 2
			if (DB["TYPE"] ~= 2) then
				DB["TYPE"] = 2
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà èçìåíåí. Âàðèàíò: {FFCC00} #2 'Öèôðû'.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà {FFCC00}#2 'Öèôðû'{FFFFFF} óæå èñïîëüçóåòñÿ!", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
			end
			saveDB()
		elseif (cmd == "type 3") then -- TYPE 3
			if (DB["TYPE"] ~= 3) then
				DB["TYPE"] = 3
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà èçìåíåí. Âàðèàíò: {FFCC00} #3.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü ïîòðåáíîñòåé ïåðñîíàæà {FFCC00}#3{FFFFFF} óæå èñïîëüçóåòñÿ!", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
			end
			saveDB()
		...
		elseif (cmd == "icons 1") then -- ICONS 1
			if (DB["ICON_STYLE"] ~= 1) then
				DB["ICON_STYLE"] = 1
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü èêîíîê èçìåíåí. Âàðèàíò: {FFCC00} #1.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü èêîíîê {FFCC00}#1{FFFFFF} óæå èñïîëüçóåòñÿ!", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
			end
			saveDB()
		elseif (cmd == "icons 2") then -- ICONS 2
			if (DB["ICON_STYLE"] ~= 2) then
				DB["ICON_STYLE"] = 2
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü èêîíîê èçìåíåí. Âàðèàíò: {FFCC00} #2.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
			else
				sampAddChatMessage(TAG .."{FFFFFF}Ñòèëü èêîíîê {FFCC00}#2{FFFFFF} óæå èñïîëüçóåòñÿ!", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
			end
			saveDB()
		-- ERROR MSG
		elseif #cmd == 0 then
			...
			addOneOffSound(0.0, 0.0, 0.0, SOUNDS["ERROR"])
			sampAddChatMessage(TAG .."{FFFFFF}Íåèçâåñòíàÿ êîìàíäà. Ââåäèòå: {FFCC00}/bars.", 0xFF3232)
		end
	end)
	----
	...
	while true do wait(0)
		...
		local posX, posY = getCursorPos()
			...
			if isKeyDown(0x01) or isKeyDown(0x0D) then -- LMB or ENTER
				...
				sampAddChatMessage(TAG .. "{FFFFFF}Ïîëîæåíèå ïîòðåáíîñòåé ïåðñîíàæà ñîõðàíåíî.", 0xFFCC00)
				addOneOffSound(0.0, 0.0, 0.0, SOUNDS["DONE"])
				setPosition = false
				saveDB()
			end
...
{AFE7FF}/bars{ffffff} - Îêíî ïîìîùè
{AFE7FF}/bars (pos)ition{ffffff} - Èçìåíåíèå ïîçèöèè
{AFE7FF}/bars type [1-3]{ffffff} - Èçìåíåíèå ñòèëÿ ïîêàçàòåëåé
{AFE7FF}/bars icons [1-2]{ffffff} - Èçìåíåíèå ñòèëÿ èêîíîê
{AFE7FF}/bars rotation(rt){ffffff} - Èçìåíåíèå ðåæèìà (Âåðòèêàëüíî èëè Ãîðèçîíòàëüíî)
{AFE7FF}/bars (num)ber{ffffff} - Ïîêàçàòü/Ñêðûòü öèôðû â ðåæèìå #1 "Ñòàìèíà"
{AFE7FF}/bars view(vw){ffffff} - Ïîêàçàòü/Ñêðûòü
{ffcc00}Ïîëåçíûå ñîâåòû:{ffffff}
{AFE7FF}[1]{ffffff} Åñëè ïîêàçàòåëè ïîêàçûâàþò "0" è {FF3232}ãîðÿò êðàñíûì{ffffff}, îòêðîéòå è çàêðîéòå èíâåíòàðü.
{AFE7FF}[2]{ffffff} Ïðè çàìåíå èêîíîê íà ñâîè, èñïîëüçóéòå ðàçìåð {ffcc00}32x32{ffffff} ïèêñåëÿ.
{ffcc00}Î ñêðèïòå:{ffffff}
{AFE7FF}Íàçâàíèå (Âåðñèÿ){ffffff}			TRINITY Bars (v1.2)
{AFE7FF}Äàòà ïîñëåäíåãî îáíîâëåíèÿ{ffffff}		02.06.2022
{AFE7FF}Àâòîð ñêðèïòà{ffffff}				eweest
{AFE7FF}Íàøå ñîîáùåñòâî{ffffff}				vk.com/gtatrinitymods
]]
